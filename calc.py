from tkinter import *   # Importar libreia grafica 'tkinter'
import parser           # 

root = Tk()             # Modulo que inicia la ventana
root.title("Calc")      # Titulo de la ventana
...
def get_operation(op):
    global i
    op_len = len(op)        # tamaño del operador en caranteres
    display.insert(i, op)
    i+=op_len               # incremento al indice con el tamaño del operador
...
def calculate():
    ...
    try:
        ...
        clear_display()
        display.insert(0, "Error: " + identifier)
...
# Botones
#       ventana texto       command=funcion                grilla  fila    columna     ocupa toda la columna    tamaño
Button( root,   text="1", command=lambda:get_numbers(1)).grid(     row=2,  column=0,   sticky=W+E,              columnspan=1)
...
root.mainloop()         # Inicia la Aplicacion
